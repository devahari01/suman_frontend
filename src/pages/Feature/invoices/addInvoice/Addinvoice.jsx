/* eslint-disable no-constant-condition */
import React, { useEffect, useContext, useState, useRef } from "react";
import "./TableStyles.css";
import { AddinvoiceContext } from "./Addinvoice.control";
import { BankSettingsContext } from "../../settings/bankSettings/BankSettings.control";
import { Link } from "react-router-dom";
import { useForm, Controller } from "react-hook-form";
import { yupResolver } from "@hookform/resolvers/yup";
import Select from "react-select";
import FeatherIcon from "feather-icons-react";
import { Form, Popconfirm, Table } from "antd";
import { commonDatacontext } from "../../../../core/commonData";
import { amountFormat } from "../../../../common/helper";
import { handleNumberRestriction } from "../../../../constans/globals";
import SignaturePadComponent from "../../../../common/SignaturePadComponent";
import DatePickerComponent from "../../datePicker/DatePicker";
import dayjs from "dayjs";
import { ApiServiceContext, InvoiceNum } from "../../../../core/core-index";
import { EditChallanContext } from "../../deliveryChallans/EditDeliveryChallan/EditChallan.control";
import { message } from "antd";


const EditableContext = React.createContext(null);
const EditableRow = ({ ...props }) => {
  const [form] = Form.useForm();
  return (
    <Form form={form} component={false}>
      <EditableContext.Provider value={form}>
        <tr {...props} />
      </EditableContext.Provider>
    </Form>
  );
};

const AddInvoice = () => {
  const { BankSettingschema } = useContext(BankSettingsContext);
  const { currencyData } = useContext(commonDatacontext);
  const [selectedSign, setselectedSign] = useState("/");
  const [product, setProduct] = useState([]);
let [defaultValue,setdefaultvalue] = useState({value:"sss"});
// const [creditNotes, setCreditNotes] = useState([]);
// const [totalTaxableAmount, setTotalTaxableAmount] = useState(0);


  const {
    id,
    dataSource,
    setDataSource,
    addInvoiceschema,
    dicountEditForm,
    submitaddInvoiceAddForm,
    handleKeyPress,
    productData,
    setProductOptiondata,
    customersData,
    bank,
    tax,
    taxableAmount,
    settaxableAmount,
    totalTax,
    settotalTax,
    totalAmount,
    settotalAmount,
    totalDiscount,
    settotalDiscount,
    roundof,
    setroundof,
    addBankSettingsForm,
    productsCloneData,
    setproductsCloneData,
    addbankpocancelModal,
    rowErr,
    setrowErr,
    setisRecurring,
    ChoosedCustomer,
    trimmedDataURL,
    setTrimmedDataURL,
    setSignatureData,
    num,
    setNum,
    type,
    referenceNo,
    taxfromcustomer,
    settaxfromcustomer,
    creditNotes,
    setCreditNotes,  // Ensure the setter function is available
    totalTaxableAmount,
    setTotalTaxableAmount,
    batchApproved, 
    setBatchApproved,
  } = useContext(AddinvoiceContext);
  // console.log("p",{customersData})
  
  let [defaultCustomer,setdefaultCustomer] = useState(customersData[0]);

useEffect(()=>{
console.log({taxfromcustomer});
},[taxfromcustomer])
useEffect(()=>{
  console.log("totalTaxableAmount",totalTaxableAmount)
},[totalTaxableAmount])
useEffect(() => {
      console.log("dataSource",dataSource)
    }, [dataSource]);
  const {
    singleChallan,
  } = useContext(EditChallanContext);
  // console.log("q",{customersData})
  // const getVendorList = async () => {
  //   const VendorList = await getData(listVendor);
  //   const newList = VendorList?.data?.map((item) => {
  //     return {
  //       id: item._id,
  //       text: item.vendor_name,
  //       value: item._id,
  //       label: item.vendor_name,
  //     };
  //   });

  //   setProduct(newList);
  // };
  const {
    handleSubmit,
    register,
    clearErrors,
    control,
    trigger,
    getValues,
    setValue: addinvsetValue,
    formState: { errors },
  } = useForm({ resolver: yupResolver(addInvoiceschema),defaultValues: { 
      products:defaultValue,customerId:defaultCustomer
    } });

  const {
    handleSubmit: bankHandle,
    control: addBankControl,
    formState: { errors: addBankerrors },
  } = useForm({ resolver: yupResolver(BankSettingschema) });

  const {
    handleSubmit: addTaxratesFormhandle,
    trigger: Edittrigger,
    setValue,
    control: addTaxratesFormControl,
    formState: { errors: addTaxratesFormErr },
  } = useForm({ resolver: yupResolver(dicountEditForm) });

  // eslint-disable-next-line no-empty-pattern
  // eslint-disable-next-line no-unused-vars
  const [paymentmethodsData, setpaymentmethodsData] = useState([
    { label: "Cash", value: "Cash" },
    { label: "Bank", value: "Bank" },
    { label: "Cheque", value: "Cheque" },
    { label: "Online", value: "Online" },
  ]);

  const [count, setCount] = useState(0);
  let editRowInputs = useRef([]);
  const EditcancelModal = useRef(null);
  const [roundofValue, setroundofValue] = useState(0);
  const { getData, postData } = useContext(ApiServiceContext);

  const changeRoundoff = (val) => {
    setroundof(val);
    if (val == true) {
      let roundValue =
        Math.round(
          Number(taxableAmount) + Number(totalTax) - Number(totalDiscount)
        ) -
        (Number(taxableAmount) + Number(totalTax) - Number(totalDiscount));
      setroundofValue(roundValue.toFixed(2));
      settotalAmount(
        Math.round(
          Number(taxableAmount) + Number(totalTax) - Number(totalDiscount)
        ).toFixed(2)
      );
    } else {
      setroundofValue((0).toFixed(2));
      settotalAmount(
        (
          Number(taxableAmount) +
          Number(totalTax) -
          Number(totalDiscount)
        ).toFixed(2)
      );
    }
  };
  useEffect(() => {
    console.log("product Value changed1:", productData);
  }, [productData]);
  useEffect(() => {
    console.log("cloned product Value changed1:", productsCloneData);
  }, [productsCloneData]);
  useEffect(() => {
    let discount = dataSource.reduce(function (tot, arr) {
      return Number(tot) + Number(arr.discount);
    }, 0);
    let rate = dataSource.reduce(function (tot, arr) {
      return Number(tot) + Number(arr.rate);
    }, 0);
    let vat = dataSource.reduce(function (tot, arr) {
      return Number(tot) + Number(arr.tax);
    }, 0);
    settaxableAmount(Number(rate).toFixed(2));
    settotalDiscount(Number(discount).toFixed(2));
    settotalTax(Number(vat).toFixed(2));
    settotalAmount((Number(rate) + Number(vat) - Number(discount)).toFixed(2));
  }, [dataSource]);

  useEffect(() => {
    if (Object.keys(ChoosedCustomer).length > 0) {
      addinvsetValue("customerId", ChoosedCustomer);
    }
  }, [ChoosedCustomer]);

  const handleBtnChanges = (value) => {
    setisRecurring(value);
  };

  const editModal = (data) => {
    setValue("tax", data?.form_updated_tax);
    setValue("rate", data?.form_updated_rate);
    setValue("discountType", data.form_updated_discounttype);
    setValue("discount", data.form_updated_discount);
    setValue("keyValue", data.key);
    setValue("productId", data.productId);
    setValue("quantity", data.quantity);
    setValue("taxInfo", data?.taxInfo);
    Edittrigger();
  };
  
  const addTableRows = (product_id) => {
    console.log("addTableRows",product_id)
    if (product_id != "") {
      let selectedProduct;
      selectedProduct = productData.find((prod) => {
        return prod?._id == `${product_id}`;
      });
      console.log({selectedProduct})

      let removeSeletctedProd;
      removeSeletctedProd = productsCloneData.filter((prod) => {
        return prod?._id != `${product_id}`;
      });
      if (!selectedProduct?.productDetails || selectedProduct.productDetails.length === 0) {
        message.warning("This product has no Quantity.");
        return;
      }
      const newData = {
        key: count,
        name: selectedProduct?.name,
        productId: selectedProduct?._id,
        units: selectedProduct?.units?.name,
        unit_id: selectedProduct?.units?._id,
        quantity: 1,
        discountType: Number(selectedProduct?.discountType),
        discount: Number(selectedProduct?.discountValue).toFixed(2),
        rate: Number(selectedProduct?.sellingPrice).toFixed(2),
        tax: taxfromcustomer,//Number(selectedProduct?.tax?.taxRate).toFixed(2),
        sku: selectedProduct?.sku,//Number(selectedProduct?.tax?.taxRate).toFixed(2),
        productDetails:selectedProduct?.productDetails,
        taxInfo: selectedProduct?.tax,
        primaryUnit:selectedProduct?.primaryUnit,
        secondaryUnit:selectedProduct?.secondaryUnit,
        numberOfPacks:selectedProduct?.numberOfPacks,
        boxQuantity:0,
        pieceQuantity:0,
        isRateFormUpadted: false,
        form_updated_discounttype: selectedProduct?.discountType,
        form_updated_discount: Number(selectedProduct?.discountValue).toFixed(
          2
        ),
        form_updated_rate: Number(selectedProduct?.sellingPrice).toFixed(2),
        form_updated_tax: taxfromcustomer,//Number(selectedProduct?.tax?.taxRate).toFixed(2),

        amount: 0,
      };

      let Calulateddicount;
      console.log("discountValue",selectedProduct?.discountValue.toFixed(2))
      let tdrateVlaue = Number(1 * selectedProduct?.sellingPrice);

      if (selectedProduct?.discountType == "2") {
        Calulateddicount = (
          tdrateVlaue *
          (selectedProduct?.discountValue / 100)
        ).toFixed(2);
      } else {
        Calulateddicount = selectedProduct?.discountValue?.toFixed(2);
      }

      let afterdiscount = (
        Number(tdrateVlaue) - Number(Calulateddicount)
      ).toFixed(2);

      // let newDataTax = (
      //   afterdiscount *
      //   (Number(selectedProduct?.tax?.taxRate) / 100)
      // ).toFixed(2);
      
      let newDataTax = (
        afterdiscount *
        (Number(taxfromcustomer) / 100)
      ).toFixed(2);

      let newDataAmount = (
        Number(selectedProduct?.sellingPrice) -
        Number(Calulateddicount) +
        Number(newDataTax)
      ).toFixed(2);

      newData.tax = newDataTax;
      newData.amount = newDataAmount;
      newData.discount = (tdrateVlaue - afterdiscount).toFixed(2);
      console.log({newData})
      console.log(newData.primaryUnit)
      console.log(newData.tax)
      console.log(newData.numberOfPacks)
      // if (newData.primaryUnit){
      //   newData.rate = newData.rate*newData.numberOfPacks;
      //   newData.tax = newData.tax*newData.numberOfPacks;
      //   newData.amount = newData.amount*newData.numberOfPacks;
      //   newData.discount = newData.discount*newData.numberOfPacks;
      // }
      setDataSource([...dataSource, newData]);
      editRowInputs.current[count] = React.createRef();
      setrowErr([
        ...rowErr,
        { field: `qtyInput${count}`, valid: true, key: count },
      ]);
      setCount(count + 1);
    }
  };
  // const addTableRows = (product_id) => {
  //   console.log("addTableRows", product_id);
    
  //   if (product_id !== "") {
  //     let selectedProduct = productData.find((prod) => prod?._id == `${product_id}`);
  //     console.log({ selectedProduct });

  //     let removeSelectedProd = productsCloneData.filter((prod) => prod?._id != `${product_id}`);
      
  //     // Calculate sum of quantities per batch
  //     const sumOfQuantity = dataSource.reduce((acc, item) => {
  //       if (item.productDetails?.length > 0) {
  //         item.productDetails.forEach((detail) => {
  //           const existingBatch = acc.find((b) => b.batchNo === detail.batchNo);
  //           if (existingBatch) {
  //             existingBatch.quantity += detail.quantity; // Accumulate quantity for the batch
  //           } else {
  //             acc.push({ batchNo: detail.batchNo, quantity: detail.quantity });
  //           }
  //         });
  //       }
  //       return acc;
  //     }, []);

  //     console.log({ sumOfQuantity });

  //     // Reduce quantity from selectedProduct.productDetails based on sumOfQuantity
  //     let updatedProductDetails = selectedProduct?.productDetails?.map((detail) => {
  //       let batch = sumOfQuantity.find((b) => b.batchNo === detail.batchNo);
  //       return {
  //         ...detail,
  //         quantity: batch ? detail.quantity - batch.quantity : detail.quantity, // Reduce batch quantity
  //       };
  //     });

  //     const newData = {
  //       key: count,
  //       name: selectedProduct?.name,
  //       productId: selectedProduct?._id,
  //       units: selectedProduct?.units?.name,
  //       unit_id: selectedProduct?.units?._id,
  //       quantity: 1,
  //       discountType: selectedProduct?.discountType,
  //       discount: Number(selectedProduct?.discountValue).toFixed(2),
  //       rate: Number(selectedProduct?.sellingPrice).toFixed(2),
  //       tax: taxfromcustomer,
  //       sku: selectedProduct?.sku,
  //       productDetails: updatedProductDetails, // Use updated productDetails
  //       taxInfo: selectedProduct?.tax,
  //       primaryUnit: selectedProduct?.primaryUnit,
  //       secondaryUnit: selectedProduct?.secondaryUnit,
  //       numberOfPacks: selectedProduct?.numberOfPacks,
  //       boxQuantity: 0,
  //       pieceQuantity: 0,
  //       isRateFormUpadted: false,
  //       form_updated_discounttype: selectedProduct?.discountType,
  //       form_updated_discount: Number(selectedProduct?.discountValue).toFixed(2),
  //       form_updated_rate: Number(selectedProduct?.sellingPrice).toFixed(2),
  //       form_updated_tax: taxfromcustomer,
  //       amount: 0,
  //     };

  //     // Calculate discount
  //     let tdrateValue = Number(1 * selectedProduct?.sellingPrice);
  //     let calculatedDiscount =
  //       selectedProduct?.discountType == "2"
  //         ? (tdrateValue * (selectedProduct?.discountValue / 100)).toFixed(2)
  //         : selectedProduct?.discountValue?.toFixed(2);

  //     let afterDiscount = (Number(tdrateValue) - Number(calculatedDiscount)).toFixed(2);
  //     let newDataTax = (afterDiscount * (Number(taxfromcustomer) / 100)).toFixed(2);
  //     let newDataAmount = (Number(selectedProduct?.sellingPrice) - Number(calculatedDiscount) + Number(newDataTax)).toFixed(2);

  //     newData.tax = newDataTax;
  //     newData.amount = newDataAmount;
  //     newData.discount = (tdrateValue - afterDiscount).toFixed(2);

  //     console.log({ newData });

  //     setDataSource([...dataSource, newData]);
  //     editRowInputs.current[count] = React.createRef();
  //     setrowErr([...rowErr, { field: `qtyInput${count}`, valid: true, key: count }]);
  //     setCount(count + 1);
  //   }
  // };
  useEffect(() => {
    console.log("inside useEffect")
    if(id){
      if (productsCloneData && Array.isArray(productsCloneData)) {
        console.log("inside useEffect1")
        productsCloneData.forEach((product) => {
          if (product?._id) {

            console.log("inside useEffect2")
            console.log({product})
            addTableRows(product._id);
            // setdefaultvalue({value:product._id})
          }
        });
      }
    }
    
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [productsCloneData]); 
useEffect(() => {
  if (!dataSource || dataSource.length === 0) return; // Ensure dataSource is not empty

  let needsUpdate = false;
  console.log("Checking if batchNo needs to be updated...");

  const updatedData = dataSource.map((item) => {
    if (!item.batchNo && item.productDetails?.length > 0) {
      needsUpdate = true;
      console.log("Updating batchNo for product:", item);
      return {
        ...item,
        batchNo: item.productDetails[0].batchNo, // Set default batchNo
        quantity: item.productDetails[0].quantity, // Ensure quantity is updated
      };
    }
    return item;
  });

  if (needsUpdate) {
    console.log("Updating dataSource...");
    setDataSource(updatedData);
  }
}, [dataSource]); // ✅ Now runs when `dataSource` changes
useEffect(() => {
  if (!dataSource || dataSource.length === 0) return;

  const firstBatchNo = dataSource[0]?.batchNo; // Get batchNo of first item

  // Check if any other item has a different batchNo
  const isBatchConsistent = dataSource.every((item) => item.batchNo === item.productDetails[0].batchNo);
  console.log('isBatchConsistent',isBatchConsistent)
  setBatchApproved(isBatchConsistent);
}, [dataSource]); // Runs when `dataSource` changes

  const handleChanges = (evnt, key) => {
    console.log("handleChanges triggered")
    console.log({evnt})
    console.log({key})
    var { name } = evnt.target;
    var value = evnt.target.value || 0;
    console.log({ name, value })
    const row = dataSource.find((item) => item.key == key);

    // reset start
    let resetRowdataBefore;
    let resetRowdataBeforerate;
    let resetRowdataBeforediscount;
    let resetRowdataBeforetax;
    resetRowdataBefore = productData.find((prod) => {
      return prod?._id == row.productId;
    });

    if (row.isRateFormUpadted) {
      resetRowdataBeforerate = row?.form_updated_rate;
      resetRowdataBeforediscount = row?.form_updated_discount
        ? row?.form_updated_discount
        : 0;
      resetRowdataBeforetax = row?.form_updated_tax ? row?.form_updated_tax : 0;
    } else {
      resetRowdataBeforerate = resetRowdataBefore?.sellingPrice;
      resetRowdataBeforediscount = resetRowdataBefore?.discountValue
        ? resetRowdataBefore?.discountValue
        : 0;
      resetRowdataBeforetax = resetRowdataBefore?.tax?.taxRate
        ? resetRowdataBefore?.tax?.taxRate
        : 0;
    }

    row.rate = resetRowdataBeforerate;
    row.discount = resetRowdataBeforediscount;
    row.tax = resetRowdataBeforetax;
    row.amount = 0;
    const newData = [...dataSource];
    const index = newData.findIndex((item) => key === item.key);
    const item = newData[index];
    newData.splice(index, 1, {
      ...item,
      ...row,
    });
    setDataSource(newData);
    // reset done

    if (value && value != "" && value != 0) {
      console.log("inside if")
      let setValidRow = rowErr.find((row) => row?.key == key);
      setValidRow.valid = true;
      const newrowData = [...rowErr];
      const rowindex = newrowData.findIndex((item) => item?.key == key);
      const rowitem = newrowData[rowindex];
      newrowData.splice(rowindex, 1, {
        ...rowitem,
        ...setValidRow,
      });
      setrowErr(newrowData);

      let temp_rate = Number(row.rate);
      let temp_discount = Number(row.discount);
      let temp_tax = Number(row.tax);
      let enteredVlaue = value;
      let enVlaue = Number(value) ;
      console.log({enVlaue})
      let discountType = Number(row.discountType);
      console.log({enteredVlaue})
      if (row.isRateFormUpadted) {
        temp_rate = row?.form_updated_rate;
        temp_discount = Number(row?.form_updated_discount);
        temp_tax = row?.form_updated_tax;
      }

      row.rate = (enVlaue * temp_rate).toFixed(2);

      let Calulateddicount;
      let tdrateVlaue = Number(enVlaue * temp_rate);

      if (discountType == "2") {
        Calulateddicount = (tdrateVlaue * (temp_discount / 100)).toFixed(2);
      } else {
        Calulateddicount = temp_discount.toFixed(2);
      }

      row.discount = Number(Calulateddicount).toFixed(2);
      let tdDiscout = tdrateVlaue - Calulateddicount;
      row.tax = (tdDiscout * (Number(temp_tax) / 100)).toFixed(2);

      row.amount = (
        Number(row.rate) +
        Number(row.tax) -
        Number(row.discount)
      ).toFixed(2);
      console.log({row})
      console.log(row.boxQuantity,row.numberOfPacks,row.pieceQuantity)
      // row.quantity = (parseInt(row.boxQuantity)*parseInt(row.numberOfPacks))+parseInt(row.pieceQuantity);
      row.quantity = enteredVlaue;
      const newData = [...dataSource];
      const index = newData.findIndex((item) => key === item.key);
      const item = newData[index];
      // if(row.primaryUnit){
      //   row.rate = row.rate*row.numberOfPacks;
      //   row.discount = row.discount*row.numberOfPacks;
      //   row.tax = row.tax*row.numberOfPacks;
      //   row.amount = row.amount*row.numberOfPacks;
      // }
      newData.splice(index, 1, {
        ...item,
        ...row,
      });
      setDataSource(newData);
    } else {
      console.log("inside else")
      let setValidRow = rowErr.find((row) => row?.key == key);
      setValidRow.valid = false;
      const newrowData = [...rowErr];
      const rowindex = newrowData.findIndex((item) => item?.key == key);
      const rowitem = newrowData[rowindex];
      newrowData.splice(rowindex, 1, {
        ...rowitem,
        ...setValidRow,
      });
      setrowErr(newrowData);
      row.rate = Number(0).toFixed(2);
      row.discount = Number(0).toFixed(2);
      row.tax = Number(0).toFixed(2);
      row.amount = Number(0).toFixed(2);
      console.log({row})
      // if(row.primaryUnit){
      //   row.rate = row.rate*row.numberOfPacks;
      //   row.discount = row.discount*row.numberOfPacks;
      //   row.tax = row.tax*row.numberOfPacks;
      //   row.amount = row.amount*row.numberOfPacks;
      // }
      
      const newData = [...dataSource];
      const index = newData.findIndex((item) => key === item.key);
      const item = newData[index];
      newData.splice(index, 1, {
        ...item,
        ...row,
      });
      setDataSource(newData);
    }
  };
  const handleCustomerChange = async (val) => {
    try {
      console.log({ "handleCustomerChange": val });
      console.log({ "handleCustomerChange_id": val._id });
      let payload = { customerId: val._id };
      let custompricing = await postData(`/drop_down/custompricing`, payload);

      if (custompricing?.code === 200) {
        // Map custom prices into an object for quick lookup
        const customPriceMap = {};
        custompricing.data.forEach(product => {
          customPriceMap[product._id] = product.customPrice || product.sellingPrice;
        });
        console.log({customPriceMap})
        console.log({productsCloneData})
        // Update only the "rate" in dataSource while preserving other fields
        const updatedDataSource = productsCloneData.map(product => {
          console.log("Updating product:", product);
          console.log("Custom price found:", customPriceMap[product._id]);
          return {
            ...product,
            sellingPrice: customPriceMap[product._id] || product.sellingPrice // Update rate if custom price exists
          }
        });
        console.log({updatedDataSource})
        setproductsCloneData([...updatedDataSource]);
        setProductOptiondata([...updatedDataSource]);

      }
      if (!val || !val.shippingAddress?.state) {
        console.error("Invalid input: Missing shipping address or state.");
        return;
      }
  
      console.log({ "handleCustomerChange state": val.shippingAddress.state });
  
      // Use `await` to handle the async function
      const response = await getData(`https://software.iyappaa.com:7005/drop_down/statetax/${val.shippingAddress.state}`);
  
      if (response?.code === 200) {
        console.log({response} );
        settaxfromcustomer(response.data[0].total_tax)
      } else {
        console.error("Error fetching state tax data:", response);
      }
      const SRresponse = await getData(`https://software.iyappaa.com:7005/customers/CustomerWithSalesreturns/${val._id}`);
  
      if (SRresponse?.success === true) {
        console.log(SRresponse.data);
        setCreditNotes(SRresponse.data);
        // const totalTaxableAmt = creditNotes.reduce((sum, note) => sum + parseFloat(note.taxableAmount || 0), 0);
        const totalTaxableAmt = SRresponse.data.reduce((sum, note) => sum + (note.taxableAmount ? parseFloat(note.taxableAmount) : 0), 0);
        console.log({totalTaxableAmt})
        setTotalTaxableAmount(totalTaxableAmt)
        // const adjustedTotalAmount = (parseFloat(totalAmount) || 0) - totalTaxableAmount - (parseFloat(totalDiscount) || 0);
        // settaxfromcustomer(response.data[0].total_tax)
      } else {
        console.error("Error fetching SRresponse data:", SRresponse);
      }

    } catch (error) {
      console.error("Error in handleCustomerChange:", error);
    }
  };
  
  const addTaxratesForm = async (data) => {
    const row = dataSource.find((item) => item.key == data.keyValue);
    let updatedTaxdata = data?.taxInfo;
    let enteredVlaue;
    if (data.quantity && data.quantity != "" && data.quantity != 0) {
      enteredVlaue = Number(data.quantity);
    }

    if (enteredVlaue != undefined) {
      // calc with min qty on suubmit
      let discountType = Number(data.discountType);
      let temp_rate = Number(data.rate);
      let temp_discount = Number(data.discount);
      // let temp_tax = Number(updatedTaxdata?.taxRate);
      let temp_tax = Number(taxfromcustomer);

      row.rate = (enteredVlaue * temp_rate).toFixed(2);

      let Calulateddicount;
      let tdrateVlaue = Number(enteredVlaue * temp_rate);

      // eslint-disable-next-line no-unused-vars
      if ((discountType = "2")) {
        Calulateddicount = (tdrateVlaue * (temp_discount / 100)).toFixed(2);
      } else {
        Calulateddicount = temp_discount.toFixed(2);
      }

      row.discount = Number(Calulateddicount).toFixed(2);
      let tdDiscout = tdrateVlaue - Calulateddicount;
      row.tax = (tdDiscout * (Number(temp_tax) / 100)).toFixed(2);
      row.amount = (
        Number(row.rate) +
        Number(row.tax) -
        Number(row.discount)
      ).toFixed(2);
      row.quantity = (row.boxQuantity*row.numberOfPacks)+row.pieceQuantity;
    } else {
      // calc without min qty on suubmit
      let discountType = Number(data.discountType);
      let temp_rate = Number(data.rate);
      let temp_discount = Number(data.discount);
      // let temp_tax = Number(updatedTaxdata?.taxRate);
      let temp_tax = Number(taxfromcustomer);

      row.rate = temp_rate.toFixed(2);

      let Calulateddicount;
      let tdrateVlaue = Number(temp_rate);

      // eslint-disable-next-line no-unused-vars
      if ((discountType = "2")) {
        Calulateddicount = (tdrateVlaue * (temp_discount / 100)).toFixed(2);
      } else {
        Calulateddicount = temp_discount.toFixed(2);
      }

      row.discount = Number(Calulateddicount).toFixed(2);
      let tdDiscout = tdrateVlaue - Calulateddicount;
      row.tax = (tdDiscout * (Number(temp_tax) / 100)).toFixed(2);
      row.amount = (
        Number(row.rate) +
        Number(row.tax) -
        Number(row.discount)
      ).toFixed(2);
      row.quantity = enteredVlaue;
    }

    row.isRateFormUpadted = true;
    row.form_updated_discount = Number(data.discount);
    row.form_updated_rate = Number(data.rate);
    // row.form_updated_tax = Number(updatedTaxdata?.taxRate);
    row.form_updated_tax = Number(taxfromcustomer);
    row.taxInfo = data.taxInfo;

    const newData = [...dataSource];
    const index = newData.findIndex((item) => data.keyValue === item.key);
    const item = newData[index];

    newData.splice(index, 1, {
      ...item,
      ...row,
    });
    setDataSource(newData);
    EditcancelModal.current.click();
  };

  var removeByAttr = function (arr, attr, value) {
    var i = arr.length;
    while (i--) {
      if (
        arr[i] &&
        // eslint-disable-next-line no-prototype-builtins
        arr[i].hasOwnProperty(attr) &&
        arguments.length > 2 &&
        arr[i][attr] == value
      ) {
        arr.splice(i, 1);
      }
    }
    return arr;
  };

  const handleDelete = (key, productId) => {
    console.log("key, productId",key, productId)
    console.log("dataSource",dataSource)
    const newData = dataSource.filter((item) => item.key !== key);
    console.log("newData",newData)
    setDataSource(newData);
    console.log("productData",productData)
    let addDeletedprod;
    addDeletedprod = productData.find((prod) => {
      return prod?._id == `${productId}`;
    });
    console.log("addDeletedprod",addDeletedprod)
    console.log("productsCloneData",productsCloneData)
    // setproductsCloneData([...productsCloneData, addDeletedprod]);
    
    let errorListarray = rowErr;
    let resterrorlists = removeByAttr(errorListarray, "key", key);
    setrowErr([...resterrorlists]);
  };

  const handleKey = (event) => {
    // eslint-disable-next-line no-unused-vars
    const { name, value } = event.target;
    if (event.keyCode === 8 || event.keyCode === 46) {
      let strVlaue = `${event.target.value}`;
      // eslint-disable-next-line no-unused-vars
      let curVal = strVlaue.replace(/:$/, "");
      //event.target.value = curVal;
    }
  };
  const handlePrimaryUnitChange = (evnt, key) => {
    // console.log({evnt})
    // console.log({key})
    const newData = [...dataSource];
    const index = newData.findIndex((item) => key === item.key);
    const item = newData[index];
    item.primaryUnit = evnt.target.value
    if(!evnt.target.value){      
      item.rate = item.rate/item.numberOfPacks;
      item.tax = item.tax/item.numberOfPacks;
      item.amount = item.amount/item.numberOfPacks;
      item.discount = item.discount/item.numberOfPacks;
    }
    newData.splice(index, 1, {
      ...item,
    });
    setDataSource(newData);
    
  }
  // const handleSecondaryUnitChange = (evnt, key) => {
  //   // console.log({evnt})
  //   // console.log({key})
  //   const newData = [...dataSource];
  //   const index = newData.findIndex((item) => key === item.key);
  //   const item = newData[index];
  //   item.secondaryUnit = evnt.target.value
  //   // if(!evnt.target.value){      
  //   //   item.rate = item.rate/item.numberOfPacks;
  //   //   item.tax = item.tax/item.numberOfPacks;
  //   //   item.amount = item.amount/item.numberOfPacks;
  //   //   item.discount = item.discount/item.numberOfPacks;
  //   // }
  //   newData.splice(index, 1, {
  //     ...item,
  //   });
  //   setDataSource(newData);
    
  // }
/*  const handleBoxQuantityChange = (evnt, key) => {
    const row = dataSource.find((item) => item.key == key);
    const { name, value } = evnt.target;
    row.quantity = value;
    const newData = [...dataSource];
    const index = newData.findIndex((item) => key === item.key);
    const item = newData[index];
    newData.splice(index, 1, {
    ...item,
    ...row,
    });
    setDataSource(newData);
    
    // Update the quantity field in state
    updateRecordQuantity(key, { name: name,value: value});
  };

  const handlePieceQuantityChange = (e, key) => {
    const newValue = e.target.value;
    
    // Update the quantity field in state
    updateRecordQuantity(recordKey, newValue);
  };

  const updateRecordQuantity = (recordKey, newValue) => {
    // setRecords((prevRecords) =>
    //   prevRecords.map((record) =>
    //     record.key === recordKey ? { ...record, quantity: newValue } : record
    //   )
    // );

    // Call handleChanges
    handleChanges({target:newValue}, recordKey);
  };*/
  const handleBatchNoChanges = (event, key) => {
    const { value } = event.target;
    const newData = [...dataSource];
    const index = newData.findIndex((item) => item.key === key);

    if (index !== -1) {
      // Find the selected batch details
      const selectedBatch = newData[index]?.productDetails?.find(
        (detail) => detail.batchNo === value
      );
      console.log({selectedBatch})
      newData[index] = {
        ...newData[index],
        batchNo: value,
      };

      setDataSource(newData);
    }
  };

  const handleBoxQuantityChange = (event, key) => {
    const { name  } = event.target;    
    var value = event.target.value;    
    // console.log({ name, value })
    const row = dataSource.find((item) => item.key == key);
    row.boxQuantity = parseInt(value) || 0;
    const newData = [...dataSource];
    const index = newData.findIndex((item) => key === item.key);
    const item = newData[index];
    
    newData.splice(index, 1, {
      ...item,
      ...row,
    });
    setDataSource(newData);
    value = Number(parseInt(row.boxQuantity*row.numberOfPacks)+row.pieceQuantity)
    updateRecordQuantity(key, { name, value});
};

const handlePieceQuantityChange = (event, key) => {
  const { name } = event.target;    
  var value = event.target.value;  
    // console.log({ name, value })
    const row = dataSource.find((item) => item.key == key);
    row.pieceQuantity = parseInt(value) || 0;
    const newData = [...dataSource];
    const index = newData.findIndex((item) => key === item.key);
    const item = newData[index];
    
    newData.splice(index, 1, {
      ...item,
      ...row,
    });
    setDataSource(newData);
    value = Number(parseInt(row.boxQuantity*row.numberOfPacks)+row.pieceQuantity)
    updateRecordQuantity(key, { name, value});
};

const updateRecordQuantity = (recordKey, newValue) => {
  // Call handleChanges to propagate updates
  handleChanges({ target: newValue }, recordKey);
};


  const defaultColumns = [
    {
      title: "Product",
      dataIndex: "name",
      render: (text, record) => {
        return record.discountType === 2 ? (
          <>
            <span>{record.name} - {record.discount}%</span>
          </>
        ) : (
          <>
            <span>{record.name} - {record.discount}$</span>
          </>
        );
      }
    },
    {
      title: "Code",
      dataIndex: "sku",
    },
    {
      title: "Tot.Qty",
      dataIndex: "batchNo",      
      render: (text, record) => {
        const totalQty = record?.productDetails?.reduce((sum, detail) => sum + (detail.quantity || 0), 0);
        return (
          <>
            <span>{totalQty}Pcs</span>
          </>
        );
      }
    },
    {
      title: "Batch No",
      dataIndex: "batchNo",      
      render: (text, record) => {
        console.log("record.numberOfPacks",record.numberOfPacks)
        return (
          <select
          className="form-control"
          onChange={(e) => handleBatchNoChanges(e, record.key)}
          defaultValue={record?.batchNo}
        >
          {record?.productDetails?.map((detail) => (
            <option key={detail.batchNo} value={detail.batchNo}>
              {detail.batchNo}-{parseInt(detail.quantity/record.numberOfPacks)}{record.primaryUnit} {detail.quantity%record.numberOfPacks}{record.secondaryUnit}
            </option>
          ))}
        </select>
          )
        
      },
    },
    {
      title: "Gross Quantity",
      dataIndex: "quantity",
      // editable: true,
      // render: (text, record) => (
      //   <div>
      //     <div className="d-flex align-items-center">
      //       <input
      //         type="text"
      //         onKeyPress={handleNumberRestriction}
      //         {...register(`qtyInput${record.key}`)}
      //         className="form-control quantity-input"
      //         onKeyDown={(e) => handleKey(e)}
      //         onKeyUp={(e) => handleKey(e)}
      //         onChange={(e) => handleChanges(e, record.key)}
      //         defaultValue={record.quantity}
      //       />
      //     </div>
      //   </div>
      // ),
      render: (text, record) => {
        // Ensure safe values (avoid undefined)
        const boxQuantity = record?.boxQuantity || 0;
        const pieceQuantity = record?.pieceQuantity || 0;
        const numberOfPacks = record?.numberOfPacks || 1;

        return <>{boxQuantity * numberOfPacks + pieceQuantity}</>;
      },
    },
    {
      title: "No of Boxes",
      dataIndex: "primaryUnit",
      render: (text, record) => {
        const allOptions = [
          { value: "", label: "None" },
          { value: "Bag", label: "BAGS" },
          { value: "Btl", label: "BOTTLES" },
          { value: "Box", label: "BOX" },
          { value: "Bdl", label: "BUNDLES" },
          { value: "Can", label: "CANS" },
          { value: "Ctn", label: "CARTONS" },
          { value: "Dzn", label: "DOZENS" },
          { value: "Gm", label: "GRAMMES" },
          { value: "Kg", label: "KILOGRAMS" },
          { value: "Ltr", label: "LITRE" },
          { value: "Mtr", label: "METERS" },
          { value: "Ml", label: "MILLILITER" },
          { value: "Nos", label: "NUMBERS" },
          { value: "Pac", label: "PACKS" },
          { value: "Prs", label: "PAIRS" },
          { value: "Pcs", label: "PIECES" },
        ];

        // Filter options based on the current primaryUnit
        const filteredOptions = allOptions.filter(
          (option) => option.value === record.primaryUnit
        );

        return (
          <div className="d-flex align-items-center">
            <input
              type="number"
              className="form-control"
              placeholder="Qty"
              min="0"
              onChange={(e) => handleBoxQuantityChange(e, record.key)}
              defaultValue={record.boxQuantity || ""}
            />{record.primaryUnit}
            {/*<select
              value={record.primaryUnit || ""}
              onChange={(e) => handlePrimaryUnitChange(e, record.key)}
              className="form-control"
              >
              {filteredOptions.map((option) => (
                <option key={option.value} value={option.value}>
                  {option.label}
                </option>
              ))}
            </select>*/}
          </div>
        );
      },
    },
    {
      title: "No of Pieces",
      dataIndex: "secondaryUnit",
      render: (text, record) => {
        const allOptions = [
          { value: "", label: "None" },
          { value: "Bag", label: "BAGS" },
          { value: "Btl", label: "BOTTLES" },
          { value: "Box", label: "BOX" },
          { value: "Bdl", label: "BUNDLES" },
          { value: "Can", label: "CANS" },
          { value: "Ctn", label: "CARTONS" },
          { value: "Dzn", label: "DOZENS" },
          { value: "Gm", label: "GRAMMES" },
          { value: "Kg", label: "KILOGRAMS" },
          { value: "Ltr", label: "LITRE" },
          { value: "Mtr", label: "METERS" },
          { value: "Ml", label: "MILLILITER" },
          { value: "Nos", label: "NUMBERS" },
          { value: "Pac", label: "PACKS" },
          { value: "Prs", label: "PAIRS" },
          { value: "Pcs", label: "PIECES" },
        ];

        const filteredOptions = allOptions.filter(
          (option) => option.value === record.secondaryUnit
        );

        return (
          <div className="d-flex align-items-center">
          <input
              type="number"
              className="form-control me-2"
              placeholder="Qty"
              min="0"
              onChange={(e) => handlePieceQuantityChange(e, record.key)}
              defaultValue={record.pieceQuantity || ""}
            />{record.secondaryUnit || ""}
          {/*<select
              value={record.secondaryUnit || ""}
              onChange={(e) => handleSecondaryUnitChange(e, record.key)}
              className="form-control"
              >
              {filteredOptions.map((option) => (
                <option key={option.value} value={option.value}>
                  {option.label}
                </option>
              ))}
            </select>*/}
            
          </div>
        );
      },
    },
    {
      title: "Rate",
      dataIndex: "rate",
      render: (text, record) => (
        <>
          {currencyData || "$"}
          {record?.rate}
        </>
      ),
    },
    {
      title: "Discount",
      dataIndex: "discount",
      render: (text, record) => (
        <>
          {currencyData || "$"}
          {record?.discount}
        </>
      ),
    },
    {
      title: "Tax",
      dataIndex: "tax",
      render: (text, record) => (
        <>
          {currencyData || "$"}
          {record?.tax}
        </>
      ),
    },
    {
      title: "Amount",
      dataIndex: "amount",
      render: (text, record) => (
        <>
          {currencyData || "$"}
          {record?.amount}
        </>
      ),
    },
    {
      title: "Action",
      render: (text, record) => (
        <div className="d-flex align-items-center">
          <Link
            to="#"
            className="btn-action-icon me-2"
            onClick={() => editModal(record)}
            data-bs-toggle="modal"
            data-bs-target="#add_discount"
          >
            <span>
              <i className="fe fe-edit">
                <FeatherIcon icon="edit" />
              </i>
            </span>
          </Link>
          <Link to="#" className="btn-action-icon">
            <Popconfirm
              title="Sure to delete?"
              onConfirm={() => handleDelete(record.key, record.productId)}
            >
              <span>
                <i className="fe fe-trash-2">
                  <FeatherIcon icon="trash-2" />
                </i>
              </span>
            </Popconfirm>
          </Link>
        </div>
      ),
    },
  ];


  const components = {
    body: {
      row: EditableRow,
    },
  };

  const columns = defaultColumns.map((col) => {
    if (!col.editable) {
      return col;
    }
    return {
      ...col,
      onCell: (record) => ({
        record,
        editable: col.editable,
        dataindex: col.dataIndex,
        title: col.title,
        // handleSave,
      }),
    };
  });

  const values = getValues();
  values.totalTax = totalTax;
  values.taxableAmount = taxableAmount;
  values.totalAmount = totalAmount;
  values.trimmedDataURL = trimmedDataURL;
  values.product = dataSource;

  //  For antd Datepicker
  const disablePastDate = (current) => {
    // Disable dates that are before today
    return current && current < dayjs().startOf("day");
  };
  const disableFutureDate = (current) => {
    // Disable dates that are after today
    return current && current > dayjs().endOf("day");
  };
  //  For antd Datepicker

  const getInvoiceNumber = async () => {
    const response = await getData(InvoiceNum);
    setNum(response?.data);
  };

  useEffect(() => {
    getInvoiceNumber();
  }, []);
  const select_Date = "Select Date"
  const onError = (errors) => {
    console.log("Form Errors:", errors); // Logs all form errors
  };
  return (
    <>
      <div className="page-wrapper">
        <div className="content container-fluid">
          <div className="content-page-header">
            <h5>Add Invoice</h5>
          </div>
          <form onSubmit={handleSubmit(submitaddInvoiceAddForm,onError)}>
            <div className="row">
              <div className="col-md-12">
                <div className="card-body">
                  <div className="form-group-item border-0 mb-0">
                    <div className="row align-item-center">
                      <div className="col-lg-4 col-md-6 col-sm-12">
                        <div className="form-group input_text">
                          <label>
                            Invoice Type<span className="text-danger"> *</span>
                          </label>
                          <div className="align-center">
                            <div className="form-control me-3">
                              <label className="custom_radio me-3 mb-0">
                              <input type="hidden" name="DCid" value={id} />
                                <input
                                  {...register("type")}
                                  type="radio"
                                  value="old"
                                  id="old_type"
                                  checked={type === "old"} // Add this to check if 'type' is 'old'
                                />
                                <span className="checkmark" /> Old
                              </label>
                            </div>
                            <div className="form-control">
                              <label className="custom_radio mb-0">
                                <input
                                  {...register("type")}
                                  type="radio"
                                  value="new"
                                  id="new_type"
                                  checked={type === "new"} // Add this to check if 'type' is 'new'
                                />
                                <span className="checkmark" /> New
                              </label>
                            </div>
                          </div>
                          <small>{errors?.type?.message}</small>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6 col-sm-12">
                        <div className="form-group input_text">
                          <label>Invoice Number</label>
                          <Controller
                            name="invoiceNumber"
                            type="number"
                            control={control}
                            // eslint-disable-next-line no-unused-vars
                            render={({ field: { value, onChange } }) => (
                              <input
                                className={`form-control ${
                                  errors?.invoiceNumber ? "error-input" : ""
                                }`}
                                type="text"
                                value={num}
                                placeholder="Enter Invoice Number"
                                autoComplete="false"
                                readOnly={true}
                                disabled={true}
                              />
                            )}
                            defaultValue=""
                          />
                          <small>{errors?.invoiceNumber?.message}</small>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6 col-sm-12">
                        <div className="form-group input_text">
                          <label>
                            Customer Name
                            <span className="text-danger"> *</span>
                          </label>
                          <ul className="form-group-plus css-equal-heights">
                            <li>
                              <Controller
                                name="customerId"
                                control={control}
                                render={({ field }) => (
                                  <Select
                                    {...field}
                                    className={`form-control react-selectcomponent w-100 ${
                                      errors?.customerId ? "error-input" : ""
                                    }`}
                                    placeholder="Select Customer"
                                    getOptionLabel={(option) =>
                                      `${option.name}`
                                    }
                                    getOptionValue={(option) => `${option._id}`}
                                    options={customersData}
                                    classNamePrefix="select_kanakku"
                                    isDisabled={
                                      Object.keys(ChoosedCustomer).length < 1
                                        ? false
                                        : true
                                    }
                                    onChange={(selectedOption) => {
                                      field.onChange(selectedOption); // Update form state
                                      handleCustomerChange(selectedOption); // Custom function
                                    }}
                                  />
                                )}
                              />
                              <small>{errors?.customerId?._id?.message}</small>
                            </li>
                            <li>
                              {Object.keys(ChoosedCustomer).length < 1 && (
                                <Link
                                  className="btn btn-primary form-plus-btn"
                                  to="/add-customer"
                                >
                                  <i className="fas fa-plus-circle" />
                                </Link>
                              )}
                            </li>
                          </ul>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6 col-sm-12">
                        <div className="form-group date-form-group">
                          <label>
                            Invoice Date<span className="text-danger"> *</span>
                          </label>
                          {/* For antd Datepicker */}
                          <Controller
                            control={control}
                            className={`form-control ${
                              errors?.invoiceDate ? "error-input" : ""
                            }`}
                            name="invoiceDate"
                            render={({ field: { value, onChange } }) => (
                              <>
                                <DatePickerComponent
                                  disabledDate={disableFutureDate}
                                  value={value}
                                  onChange={onChange}
                                />
                                <small>{errors?.invoiceDate?.message}</small>
                              </>
                            )}
                          ></Controller>
                          {/* For antd Datepicker */}
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6 col-sm-12">
                        <div className="form-group date-form-group">
                          <label>
                            Due Date<span className="text-danger"> *</span>
                          </label>
                          <Controller
                            name="dueDate"
                            control={control}
                            className={`form-control ${
                              errors?.dueDate ? "error-input" : ""
                            }`}
                            render={({ field: { value, onChange } }) => (
                              <DatePickerComponent
                                disabledDate={disablePastDate}
                                value={value}
                                onChange={onChange}
                              />
                            )}
                          ></Controller>
                          <small>{errors?.dueDate?.message}</small>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6 col-sm-12">
                        <div className="form-group input_text">
                          <label>Reference Number</label>
                          <Controller
                            name="referenceNo"
                            type="number"
                            control={control}
                            defaultValue={referenceNo}
                            render={({ field: { value, onChange } }) => (
                              <input
                                className={`form-control ${
                                  errors?.referenceNo ? "error-input" : ""
                                }`}
                                type="text"
                                onKeyPress={handleNumberRestriction}
                                value={referenceNo?referenceNo:(value || "")}
                                onChange={onChange}
                                placeholder="Enter Reference Number"
                                autoComplete="false"
                              />
                            )}
                          />
                          <small>{errors?.referenceNo?.message}</small>
                        </div>
                      </div>
                      {/*<div className="col-lg-4 col-md-6 col-sm-12">
                        <div className="form-group input_text">
                          <label>Discount Amount</label>
                          <Controller
                            name="discountAmount"
                            type="number"
                            control={control}
                            defaultValue=""
                            rules={{
                              required: "Discount is required",
                              min: { value: 0.01, message: "Discount must be greater than 0" }
                            }}
                            render={({ field: { value, onChange } }) => (
                              <input
                                className={`form-control ${errors?.discountAmount ? "error-input" : ""}`}
                                type="number"
                                value={value || ""}
                                onChange={(e) => {
                                  let discountValue = parseFloat(e.target.value);
                                  if (discountValue > 0) {
                                    settotalDiscount(discountValue.toFixed(2));
                                  }
                                  onChange(e);
                                }}
                                placeholder="Enter Discount Amount"
                                autoComplete="off"
                              />
                            )}
                          />
                          <small className="text-danger">{errors?.discountAmount?.message}</small>
                        </div>
                      </div>*/}


                      <div className="col-lg-4 col-md-6 col-sm-12">
                        <div className="form-group input_text">
                          <label>
                            Payment Method
                            <span className="text-danger"> *</span>
                          </label>
                          <Controller
                            name="payment_method"
                            control={control}
                            render={({ field }) => (
                              <Select
                                {...field}
                                className={`form-control react-selectcomponent w-100 ${
                                  errors?.payment_method ? "error-input" : ""
                                }`}
                                placeholder="Select Payment Method"
                                options={paymentmethodsData}
                                classNamePrefix="select_kanakku"
                              />
                            )}
                          />
                          <small>
                            {errors?.payment_method?.value?.message}
                          </small>
                        </div>
                      </div>
                      
                      <div className="col-lg-12">
                        <div className="form-group">
                          <label>
                            Products<span className="text-danger"> *</span>
                          </label>
                          {/* addTableRows */}
                          <ul className="form-group-plus css-equal-heights">
                            <li>
                              <Controller
                                name="products"
                                control={control}
                                render={({ field: { onChange } }) => (
                                  <Select
                                    className={`react-selectcomponent form-control w-100 ${
                                      errors?.products ? "error-input" : ""
                                    }`}
                                    options={productsCloneData}
                                    placeholder="Select Products"
                                    value={""}
                                    type="number"
                                    classNamePrefix="select_kanakku"
                                    onChange={(e) => {
                                      console.log({e})
                                      onChange(e);
                                      trigger("products");
                                      addTableRows(e?.value);
                                    }}
                                  />
                                )}
                              />
                              <small className="text-danger">
                                {errors?.products?.value?.message}
                              </small>
                            </li>
                            <li>
                              <Link
                                className="btn btn-primary form-plus-btn"
                                to="/add-product"
                              >
                                <i className="fas fa-plus-circle" />
                              </Link>
                            </li>
                          </ul>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="form-group-item add-invoice-table">
                    <div className="card-table">
                      <div className="card-body product-list category">
                        <div className="table-responsive">
                          <Table
                            components={components}
                            rowClassName={() => "editable-row"}
                            bordered
                            dataSource={dataSource}
                            columns={columns}
                            pagination={{ position: ["none", "none"] }}
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="form-group-item border-0 p-0">
                    <div className="row">
                      <div className="col-xl-6 col-lg-12">
                        <div className="form-group-bank">
                          <div className="row align-items-center">
                            <div className="col-md-8">
                              <div className="form-group">
                                <label>Select Bank</label>
                                <Controller
                                  name="bank"
                                  control={control}
                                  render={({ field }) => (
                                    <>
                                      <Select
                                        {...field}
                                       
                                        className={` w-100`}
                                        placeholder="Select Bank"
                                        getOptionLabel={(option) =>
                                          `${option.bankName}`
                                        }
                                        getOptionValue={(option) =>
                                          `${option._id}`
                                        }
                                        options={bank}
                                        classNamePrefix="select_kanakku"
                                      />
                                      <small className="text-danger">
                                        {errors?.bank?._id.message}
                                      </small>
                                    </>
                                  )}
                                />
                              </div>
                            </div>
                            <div className="col-md-4">
                              <div className="form-groups">
                                <Link
                                  className="btn btn-primary"
                                  to="#"
                                  data-bs-toggle="modal"
                                  data-bs-target="#bank_details"
                                >
                                  Add Bank
                                </Link>
                              </div>
                            </div>
                          </div>
                          <div className="form-group notes-form-group-info">
                            <label>Notes</label>
                            <Controller
                              name="notes"
                              type="text"
                              control={control}
                              render={({ field: { value, onChange } }) => (
                                <textarea
                                  className={`form-control ${
                                    errors?.notes ? "error-input" : ""
                                  }`}
                                  type="text"
                                  value={value || ""}
                                  onChange={onChange}
                                  placeholder="Enter Notes"
                                  autoComplete="false"
                                  onKeyPress={handleKeyPress}
                                />
                              )}
                              defaultValue=""
                            />
                            <small className="text-danger">
                              {errors?.notes?.message}
                            </small>
                          </div>
                          <div className="form-group input_text notes-form-group-info mb-0">
                            <label>Terms and Conditions</label>
                            <Controller
                              name="termsAndCondition"
                              type="text"
                              control={control}
                              render={({ field: { value, onChange } }) => (
                                <textarea
                                  className={`form-control ${
                                    errors?.termsAndCondition
                                      ? "error-input"
                                      : ""
                                  }`}
                                  type="text"
                                  value={value || ""}
                                  onChange={onChange}
                                  placeholder="Enter Terms and Conditions"
                                  autoComplete="false"
                                  // onKeyPress={handleKeyPress}
                                />
                              )}
                              defaultValue=""
                            />
                            <small className="text-danger">
                              {errors?.termsAndCondition?.message}
                            </small>
                          </div>
                        </div>
                      </div>
                      <div className="col-xl-6 col-lg-12">
                        <div className="form-group-bank">
                          <div className="invoice-total-box">
                            <div className="invoice-total-inner">
                              <p>
                                Amount{" "}
                                <span>
                                  {currencyData ? currencyData : "$"}
                                  {amountFormat(taxableAmount)}
                                </span>
                              </p>
                              <p>
                                Discount{" "}
                                <span>
                                  {currencyData ? currencyData : "$"}
                                  {amountFormat(totalDiscount)}
                                </span>
                              </p>
                              <p>
                                Tax{" "}
                                <span>
                                  {currencyData ? currencyData : "$"}
                                  {amountFormat(totalTax)}
                                </span>
                              </p>
                              <div className="status-toggle justify-content-between">
                                <div className="d-flex align-center">
                                  <p>Round Off</p>
                                  <Controller
                                    name="roundof"
                                    control={control}
                                    render={({
                                      field: { value, onChange },
                                    }) => (
                                      <>
                                        <input
                                          id="rating_1"
                                          className="check"
                                          type="checkbox"
                                          checked={roundof}
                                          value={value || ""}
                                          onChange={(val) => {
                                            onChange(val);
                                            trigger("roundof");
                                            changeRoundoff(val.target.checked);
                                            setroundof(!roundof);
                                          }}
                                        />
                                      </>
                                    )}
                                  />
                                  <label
                                    htmlFor="rating_1"
                                    className="checktoggle checkbox-bg"
                                  >
                                    checkbox
                                  </label>
                                </div>
                                <span>
                                  {currencyData ? currencyData : "$"}
                                  {amountFormat(roundofValue)}
                                </span>
                              </div>
                              {creditNotes && creditNotes.length > 0 && (
                                <table className="table mt-3">
                                  <thead>
                                    <tr>
                                      <th>SR ID</th>
                                      <th>SR Amount</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    {creditNotes.map((note) => (
                                      <tr key={note._id}>
                                        <td>{note.credit_note_id}</td>
                                        <td>{note.taxableAmount}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
                              )}
                            </div>
                            <div className="invoice-total-footer">
                              <h4>
                                Total Amount{" "}
                                <span>
                                  {currencyData ? currencyData : "$"}
                                  {/*{amountFormat(adjustedTotalAmount)}*/}
                                  {amountFormat(totalAmount-totalDiscount-totalTaxableAmount)}
                                </span>
                              </h4>
                            </div>
                          </div>
                          <SignaturePadComponent
                            setValue={addinvsetValue}
                            register={register}
                            trigger={trigger}
                            formcontrol={control}
                            errors={errors}
                            clearErrors={clearErrors}
                            setTrimmedDataURL={setTrimmedDataURL}
                            trimmedDataURL={trimmedDataURL}
                            setSignatureData={setSignatureData}
                            handleKeyPress={handleKeyPress}
                            selectedSign={selectedSign}
                            setselectedSign={setselectedSign}
                            data={{}}
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="text-end">
                
                  <Link
                    to="/invoice-list"
                    type="reset"
                    className="btn btn-primary cancel me-2"
                  >
                    Cancel
                  </Link>
                  <button type="submit" className="btn btn-primary">
                    Save
                  </button>
                </div>
              </div>
            </div>
            {/* </div> */}
          </form>
        </div>
      </div>

      <div className="modal custom-modal fade" id="add_discount" role="dialog">
        <div className="modal-dialog modal-dialog-centered modal-md">
          <form onSubmit={addTaxratesFormhandle(addTaxratesForm)}>
            <div className="modal-content">
              <div className="modal-header border-0 pb-0">
                <div className="form-header modal-header-title text-start mb-0">
                  <h4 className="mb-0">Add Tax &amp; Discount</h4>
                </div>
                <button
                  type="button"
                  className="close"
                  data-bs-dismiss="modal"
                  aria-label="Close"
                >
                  <span className="align-center" aria-hidden="true">
                    ×
                  </span>
                </button>
              </div>
              <div className="modal-body">
                <div className="row">
                  <div className="col-lg-12 col-md-12">
                    <div className="form-group input_text">
                      <label>Rate</label>
                      <Controller
                        name={`keyValue`}
                        type="hidden"
                        control={addTaxratesFormControl}
                        render={({ field: { value } }) => (
                          <input type="hidden" value={value || ""} />
                        )}
                      />
                      <Controller
                        name={`productId`}
                        type="hidden"
                        control={addTaxratesFormControl}
                        render={({ field: { value } }) => (
                          <input type="hidden" value={value || ""} />
                        )}
                      />
                      <Controller
                        name={`quantity`}
                        type="hidden"
                        control={addTaxratesFormControl}
                        render={({ field: { value } }) => (
                          <input type="hidden" value={value || ""} />
                        )}
                      />
                      <Controller
                        name={`discountType`}
                        type="hidden"
                        control={addTaxratesFormControl}
                        render={({ field: { value } }) => (
                          <input type="hidden" value={value || ""} />
                        )}
                      />
                      <Controller
                        name={`tax`}
                        type="hidden"
                        control={addTaxratesFormControl}
                        render={({ field: { value } }) => (
                          <input type="hidden" value={value || ""} />
                        )}
                      />
                      <Controller
                        name={`rate`}
                        type="number"
                        control={addTaxratesFormControl}
                        render={({ field: { value, onChange } }) => (
                          <input
                            className={`form-control ${
                              errors?.rate ? "error-input" : ""
                            }`}
                            type="number"
                            value={value || ""}
                            onChange={onChange}
                            placeholder="Enter Rate"
                            autoComplete="false"
                          />
                        )}
                      />
                      <small>{addTaxratesFormErr?.rate?.message}</small>
                    </div>
                  </div>
                  <div className="col-lg-12 col-md-12">
                    <div className="form-group input_text">
                      <label>Discount Amount</label>
                      <Controller
                        name={`discount`}
                        type="number"
                        control={addTaxratesFormControl}
                        render={({ field: { value, onChange } }) => (
                          <input
                            className={`form-control ${
                              errors?.discount ? "error-input" : ""
                            }`}
                            type="number"
                            value={value || ""}
                            onChange={onChange}
                            placeholder="Enter discount"
                            autoComplete="false"
                          />
                        )}
                      />
                      <small>{addTaxratesFormErr?.discount?.message}</small>
                    </div>
                  </div>
                  <div className="col-lg-12 col-md-12">
                    <div className="form-group mb-0 input_text">
                      <label>Tax</label>
                      <Controller
                        name="taxInfo"
                        control={addTaxratesFormControl}
                        render={({ field }) => (
                          <Select
                            {...field}
                           
                            className={`react-selectcomponent form-control`}
                            placeholder="Select Tax"
                            getOptionLabel={(option) =>
                              `${option.name} (${option.taxRate}%)`
                            }
                            getOptionValue={(option) => `${option._id}`}
                            options={tax}
                            classNamePrefix="select_kanakku"
                          />
                        )}
                      />
                    </div>
                  </div>
                </div>
              </div>
              <div className="modal-footer">
                <Link
                  to="#"
                  ref={EditcancelModal}
                  data-bs-dismiss="modal"
                  className="btn btn-primary paid-cancel-btn me-2"
                >
                  Back
                </Link>
                <button
                  className="btn btn-primary paid-continue-btn"
                  type="submit"
                >
                  Save
                </button>
              </div>
            </div>
          </form>
        </div>
      </div>

      <div
        className="modal custom-modal fade"
        id="delete_discount"
        role="dialog"
      >
        <div className="modal-dialog modal-dialog-centered modal-md">
          <div className="modal-content">
            <div className="modal-header border-0 justify-content-center pb-0">
              <div className="form-header modal-header-title text-center mb-0">
                <h4 className="mb-2">Delete Products</h4>
                <p>Are you sure want to delete?</p>
              </div>
            </div>
            <div className="modal-body">
              <div className="modal-btn delete-action">
                <div className="row">
                  <div className="col-6">
                    <Link
                      to="#"
                      data-bs-dismiss="modal"
                      className="btn btn-primary paid-continue-btn"
                    >
                      Delete
                    </Link>
                  </div>
                  <div className="col-6">
                    <Link
                      to="#"
                      data-bs-dismiss="modal"
                      className="btn btn-primary paid-cancel-btn"
                    >
                      Cancel
                    </Link>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="modal custom-modal fade" id="bank_details" role="dialog">
        <div className="modal-dialog modal-dialog-centered modal-md">
          <form onSubmit={bankHandle(addBankSettingsForm)}>
            <div className="modal-content">
              <div className="modal-header border-0 pb-0">
                <div className="form-header modal-header-title text-start mb-0">
                  <h4 className="mb-0">Add Bank Details</h4>
                </div>
                <button
                  type="button"
                  className="close"
                  data-bs-dismiss="modal"
                  aria-label="Close"
                >
                  <span className="align-center" aria-hidden="true">
                    ×
                  </span>
                </button>
              </div>
              <div className="modal-body">
                <div className="row">
                  <div className="col-lg-12 col-md-12">
                    <div className="form-group input_text">
                      <label>
                        Bank Name <span className="text-danger">*</span>
                      </label>
                      <Controller
                        name="bankName"
                        type="text"
                        control={addBankControl}
                        render={({ field: { value, onChange } }) => (
                          <input
                            className={`form-control ${
                              addBankerrors?.bankName ? "error-input" : ""
                            }`}
                            type="text"
                            value={value || ""}
                            onChange={onChange}
                            placeholder="Enter Bank Name"
                            autoComplete="false"
                          />
                        )}
                        defaultValue=""
                      />
                      <small>{addBankerrors?.bankName?.message}</small>
                    </div>
                  </div>
                  <div className="col-lg-12 col-md-12">
                    <div className="form-group input_text">
                      <label>
                        Account Number <span className="text-danger">*</span>
                      </label>
                      <Controller
                        name="accountNumber"
                        type="number"
                        control={addBankControl}
                        render={({ field: { value, onChange } }) => (
                          <input
                            className={`form-control ${
                              addBankerrors?.accountNumber ? "error-input" : ""
                            }`}
                            type="number"
                            value={value || ""}
                            onChange={onChange}
                            placeholder="Enter Account Number"
                            autoComplete="false"
                          />
                        )}
                        defaultValue=""
                      />
                      <small>{addBankerrors?.accountNumber?.message}</small>
                    </div>
                  </div>
                  <div className="col-lg-12 col-md-12">
                    <div className="form-group input_text">
                      <label>
                        Account Holder Name{" "}
                        <span className="text-danger">*</span>
                      </label>
                      <Controller
                        name="name"
                        type="text"
                        control={addBankControl}
                        render={({ field: { value, onChange } }) => (
                          <input
                            className={`form-control ${
                              addBankerrors?.name ? "error-input" : ""
                            }`}
                            type="text"
                            value={value || ""}
                            onChange={onChange}
                            placeholder="Enter Account Holder Name"
                            autoComplete="false"
                          />
                        )}
                        defaultValue=""
                      />
                      <small>{addBankerrors?.name?.message}</small>
                    </div>
                  </div>
                  <div className="col-lg-12 col-md-12">
                    <div className="form-group input_text">
                      <label>
                        Branch Name <span className="text-danger">*</span>
                      </label>
                      <Controller
                        name="branch"
                        type="text"
                        control={addBankControl}
                        render={({ field: { value, onChange } }) => (
                          <input
                            className={`form-control ${
                              addBankerrors?.branch ? "error-input" : ""
                            }`}
                            type="text"
                            value={value || ""}
                            onChange={onChange}
                            placeholder="Enter branch Name"
                            autoComplete="false"
                          />
                        )}
                        defaultValue=""
                      />
                      <small>{addBankerrors?.branch?.message}</small>
                    </div>
                  </div>
                  <div className="col-lg-12 col-md-12">
                    <div className="form-group input_text mb-0">
                      <label>
                        IFSC Code <span className="text-danger">*</span>
                      </label>
                      <Controller
                        name="IFSCCode"
                        type="text"
                        control={addBankControl}
                        render={({ field: { value, onChange } }) => (
                          <input
                            className={`form-control ${
                              addBankerrors?.IFSCCode ? "error-input" : ""
                            }`}
                            type="text"
                            value={value || ""}
                            onChange={onChange}
                            placeholder="Enter IFSC Code"
                            autoComplete="false"
                          />
                        )}
                        defaultValue=""
                      />
                      <small>{addBankerrors?.IFSCCode?.message}</small>
                    </div>
                  </div>
                </div>
              </div>
              <div className="modal-footer">
                <Link
                  to="#"
                  ref={addbankpocancelModal}
                  data-bs-dismiss="modal"
                  className="btn btn-primary paid-cancel-btn me-2"
                >
                  Cancel
                </Link>
                <button
                  className="btn btn-primary paid-continue-btn"
                  type="submit"
                >
                  Save
                </button>
              </div>
            </div>
          </form>
        </div>
      </div>
    </>
  );
};
export default AddInvoice;
