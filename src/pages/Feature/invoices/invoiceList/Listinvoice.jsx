...
const Listinvoice = () => {
  ...
  const handleApprove = async (record) => {
    ...
    try {
      const response = await postData("/invoice/approve", { invoiceId: record._id });
      if (response.code == 200) {
        // successToast("Invoice Approved Successfully");
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <button
          className={`btn btn-${record.batchApproved ? "success" : "warning"}`}
          onClick={() => handleApprove(record)}
        >
          {record.batchApproved ? "Approved" : "Approve"}
        </button>
      ),
    },
    {
