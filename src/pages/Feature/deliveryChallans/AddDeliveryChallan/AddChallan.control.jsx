...
const AddChallanComponentController = (props) => {
  ...
  const onSubmit = async (data) => {
    ...
    let finalArray = [];
    // sending.map((item) => {
    //   let finished = productService.map((ele) => {
    //     if (item._id === ele._id) {
    ...
    //   });
    // });
    finalArray = productService;
    console.log(finalArray);
    const formData = new FormData();
    for (let i = 0; i < finalArray.length; i++) {
      formData.append(`items[${i}][productId]`, finalArray[i]._id);
      formData.append(`items[${i}][sku]`, finalArray[i].sku);
      formData.append(`items[${i}][quantity]`, finalArray[i].quantity);
      formData.append(`items[${i}][batchNo]`, finalArray[i].batchNo);
      ...
      formData.append(`items[${i}][primaryUnit]`, finalArray[i]?.primaryUnit);
      formData.append(`items[${i}][numberOfPacks]`, finalArray[i]?.numberOfPacks);
      formData.append(`items[${i}][secondaryUnit]`, finalArray[i]?.secondaryUnit);
      formData.append(`items[${i}][pieceQuantity]`, finalArray[i]?.pieceQuantity);
      formData.append(`items[${i}][boxQuantity]`, finalArray[i]?.boxQuantity);
      formData.append(
  ...
  const calculateAmount = (record) => {
    ...
    var countTaxFinal = productServiceCopy.find((ele) => ele._id == record?._id)
      ?.tax?.taxRate;
    console.log('countTaxFinal',countTaxFinal)
    let discountFinal =
