...
const AddChallan = () => {
  ...
  const SelectedList = (value) => {
    ...
    selectList = { ...selectList, key: count };
    add.push(selectList);
  ...
  useEffect(() => {
    ...
    console.log("productOptionCopy got changed", productOptionCopy);
  }, [productService, productOptionCopy]);
...
// To add the selected option to table and remove that option from the dropdown
  ...
  const handleDelete = (id) => {
    let deleted = productService.filter((ele) => ele.key !== id);
    let addDropDown = productService?.find((ele) => ele.key == id);
  ...
  // Delete individual Products
  const handlePrimaryUnitChange = (e, id) => {
    // console.log({id})
    // console.log(e.target.value);
    // console.log(productOptionCopy);
    const selectedunit = productOptionCopy.map(ele=>{
      console.log({ele})
      if (ele.id == id) {
        return { ...ele, primaryUnit:e.target.value };
      } else {
        return { ...ele };
      }
    })
    console.log({selectedunit})
    setProductOptionCopy(selectedunit)
  };
  ...
  /*const handleUnitChange = (e, key) => {
    ...
    let updatedProductOptions = productOptionCopy.map((ele) => {
      if (ele.key === key) {
        return { ...ele, quantity: Number(e.target.value) };
      }
      return ele;
    ...
    setProductOptionCopy(updatedProductOptions);
  };*/
  const handleUnitChange = (e, index) => {
    ...
    // Create a new array with the updated quantity at the correct index
    const updatedProductOptions = [...productService];
    updatedProductOptions[index] = {
      ...updatedProductOptions[index],
      quantity: newQuantity,
    };

    // setProductOptionCopy(updatedProductOptions);
    setProductService(updatedProductOptions)
  };

  const handleBoxQuantityChange = (event, key) => {
    console.log("Updating quantity for key:", key);

    let newQuantity = parseInt(event.target.value, 10);

    // Validate if newQuantity is NaN (not a number) and set it to 0
    ...
    // Find the item by key (_id)
    const index = updatedProductService.findIndex((item) => item.key === key);
    if (index === -1) return; // Exit if item is not found

    // Update boxQuantity first, then calculate the new quantity
    updatedProductService[index] = {
      ...updatedProductService[index],
      boxQuantity: newQuantity,
      quantity: newQuantity * updatedProductService[index].numberOfPacks + (updatedProductService[index].pieceQuantity || 0),
    };
  ...
  const handlePieceQuantityChange = (event, key) => {
    ...
    updatedProductService[index] = {
      ...updatedProductService[index],
      pieceQuantity: newQuantity,
      quantity: ((updatedProductService[index].boxQuantity || 0) * updatedProductService[index].numberOfPacks) + (newQuantity || 0),
    };
...
const updateRecordQuantity = (recordKey, newValue) => {
  ...
  handleUnitChange({ target: newValue }, recordKey);
};
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        const index = productService.findIndex((ele) => ele._id === record?._id);
        return (
          <input
            type="text"
            ...
            onChange={(e) => handleUnitChange(e, record.key)}
          />
        );
      },
    },
    {
      ...
      render: (text, record) => {
        console.log({ record });
        const index = productService.findIndex((ele) => ele.id === record?._id);
        return (
                        ...
                        <div className="form-group">
                          ...
                          <ul className="form-group-plus css-equal-heights">
                            ...
                            <li>
                              ...
                              {/* <Link
                                className={`btn btn-primary form-plus-btn ${!customer ? "disabled" : ""}`}
                          ...
                          {/* {!customer && (
                            <small className="text-danger">
                              Please select a customer to enable product selection.
                            </small>
                          )} */}
                        </div>
                      </div>
