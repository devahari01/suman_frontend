  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        return (
          <select
            value= {productOptionCopy.find((ele) => {
              console.log("Checking record:", record); // Logs each element during the find operation
              console.log("Checking element:", ele);
              return ele.id == record._id;
            })?.primaryUnit || ""}
            onChange={(e) => handlePrimaryUnitChange(e, record._id)}
            className="form-control"
          >
