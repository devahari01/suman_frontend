...
const handlePrimaryUnitChange = (evnt, key) => {
  ...
  const handleChanges = (evnt, key) => {
    ...
    if (value && value != "" && value != 0) {
      ...
      // row.weight = enteredVlaue*row.weight;
      const newData = [...dataSource];
      const index = newData.findIndex((item) => key === item.key);
      const item = newData[index];
      if(row.primaryUnit){
  ...
  const defaultColumns = [
    ...
    //   render: (text, record) => (
    //     <>
    //     if(record.primaryUnit)
    //       {record?.quantity*record?.weight }{record?.units}
    //     else{
    //       {record?.quantity*record?.weight*record?.numberOfPacks }{record?.units}
    //     }
    //     </>
    //   ),
    // },
    ...
    },
    {
      ...
      render: (text, record) => {
        const allOptions = [
          ...
          { value: "Gm", label: "GRAMMES (Gm)" },
          { value: "Kg", label: "KILOGRAMS (Kg)" },
          { value: "Ltr", label: "LITRE (Ltr)" },
          { value: "Mtr", label: "METERS (Mtr)" },
          { value: "Ml", label: "MILLILITER (Ml)" },
          { value: "Nos", label: "NUMBERS (Nos)" },
          { value: "Pac", label: "PACKS (Pac)" },
          { value: "Prs", label: "PAIRS (Prs)" },
          { value: "Pcs", label: "PIECES (Pcs)" },
        ];

        // Filter options based on the current primaryUnit
        const filteredOptions = allOptions.filter(
          (option) => option.value === "" || option.value === record.primaryUnit
        );
