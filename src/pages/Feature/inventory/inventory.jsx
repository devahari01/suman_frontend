...
const Inventory = () => {
  ...
  const fetchInventoryDetails = async (id) => {
    const url = `${process.env.REACT_APP_BACKEND_URL}goods/viewstockdetails/${id}`;
    try {
      const response = await getData(url);
      if (response?.data) {
        setInventoryDetails(response); // Store fetched data in state
      }
    } catch (error) {
      ...
      render: (text, record) => (
        <div className="d-flex align-items-center">
          <button
            ...
            onClick={() => fetchInventoryDetails(record?.inventory_Info?.[0]?.productId
            ? record?.inventory_Info?.[0]?.productId
            : 0)}
          >
            View
          </button>
  ...
  return (
    <>
      ...
      <div className="modal custom-modal fade" id="inventoryDetailsModal" role="dialog">
        <div className="modal-dialog modal-dialog-centered modal-lg">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">Inventory Batch Details</h5>
              <button type="button" className="close" data-bs-dismiss="modal">
                <span>&times;</span>
              </button>
            </div>
            <div className="modal-body">
              {inventoryDetails?.data?.length > 0 ? (
                <table className="table table-bordered">
                  ...
                  <tbody>
                    {inventoryDetails.data.map((batch) => (
                      <tr key={batch._id}>
                        <td>{batch.batchNo}</td>
                        <td>{new Date(batch.mfgDate).toLocaleDateString()}</td>
                        <td>{new Date(batch.expDate).toLocaleDateString()}</td>
