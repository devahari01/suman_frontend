...
const EditproductComponentController = (props) => {
  ...
  const getmasterDetails = async () => {
    try {
      let selectedProdIds = [];
      const viewPodata = await getData(`${process.env.REACT_APP_BACKEND_URL}goods/viewProduct/${id}`);
  ...
  const submiteditPOForm = async (data) => {
    ...
    try {
      const response = await putData(`${process.env.REACT_APP_BACKEND_URL}goods/updateProduct/${poData._id}`, formData);
      console.log(response)
      if (response.code === 200) {
        successToast("Goods inward Updated Successfully");
        navigate("/goods-inward");
      }
      return response;
    } catch (error) {
      //
