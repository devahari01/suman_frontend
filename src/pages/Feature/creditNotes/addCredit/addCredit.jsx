...
const AddCredit = () => {
  ...
  const handleStatusChange = (event, key) => {
    ...
    console.log(`Status for record ${key} changed to:`, value);
  };
  const columns = [
    {
      title: "Product",
        ...
        >
          {record?.productDetails?.map((detail) => (
            <option key={detail.batchNo} value={detail.batchNo}>
              {detail.batchNo}
              {/*-{parseInt(detail.quantity/record.numberOfPacks)}{record.primaryUnit} {detail.quantity%record.numberOfPacks}{record.secondaryUnit}*/}
            </option>
          ))}
        </select>
          )
    ...
    },
    {
      ...
      dataIndex: "status",
      render: (text, record) => (
        <select className="form-control" onChange={(e) => handleStatusChange(e, record._id)}>
